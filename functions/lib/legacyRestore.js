...
exports.processVolunteerAttendanceCredits = exports.mergeCertificatesPdf = exports.generateCertificates = void 0;
// @ts-nocheck
...
exports.generateCertificates = (0, https_1.onCall)({ cors: true, region: 'us-central1' }, async (request) => {
    const { eventName, templateSet, addPoints, points } = request.data || {};
    if (!eventName)
        throw new https_1.HttpsError('invalid-argument', 'Falta eventName');
    ...
    const existingByEmailRole = new Set(certsSnap.docs.map((d) => {
    ...
    if (addPointsEnabled && (!Number.isFinite(pointsPerCertificate) || pointsPerCertificate <= 0)) {
        throw new https_1.HttpsError('invalid-argument', 'Puntos inválidos');
    }
    ...
    try {
        ...
        if (eventDocSnap && eventDocSnap.exists) {
            ...
            if (isVolunteerEvent)
                inferredTemplateSet = 'VOLUNTARIADO';
        }
        if (!eventDate) {
            ...
            if (!adminQuery.empty) {
                ...
                if (ad?.toDate)
                    ...
                    eventDate = new Date(ad);
                else if (ad instanceof Date)
                    eventDate = ad;
                const adminName = String(adminQuery.docs[0].get('name') || eventName || '').toLowerCase();
                if (adminName.includes('voluntariado'))
                    inferredTemplateSet = 'VOLUNTARIADO';
            }
        }
    }
    ...
    for (const r of regs) {
        if (!isPresent(r.statusAsistencia))
            continue; // solo presentes
        ...
        if (existingByEmailRole.has(`${email}|${roleLower}|${tempEventTitle}`)) {
            skipped++;
            continue;
        }
        try {
            ...
            let pdfDoc;
            if (templatePick?.buf) {
                pdfDoc = await pdf_lib_1.PDFDocument.load(templatePick.buf);
            }
            else {
            ...
            const tryEmbed = async (buf, key) => {
                try {
                    ...
                    console.warn('Font embed failed', key, e?.message || e);
                }
                return null;
            };
            ...
            if (boldFont)
                fontMap['Libre Franklin:bold'] = boldFont;
            if (regularFont)
                fontMap['Libre Franklin:normal'] = regularFont;
            if (italicFont)
                fontMap['Libre Franklin:italic'] = italicFont;
            if (boldItalicFont)
                fontMap['Libre Franklin:bolditalic'] = boldItalicFont;
            ...
            const hexToRgb = (hex) => {
                ...
                return (0, pdf_lib_1.rgb)(parseInt(m[1], 16) / 255, parseInt(m[2], 16) / 255, parseInt(m[3], 16) / 255);
            };
            const pickFont = (style) => {
                ...
                if (wantsLibre) {
                    if (italic && weight >= 700) {
                        ...
                        if (fontMap['Libre Franklin:bolditalic'])
                            return fontMap['Libre Franklin:bolditalic'];
                        if (fontMap['Libre Franklin:italic'])
                            return fontMap['Libre Franklin:italic'];
                        if (fontMap['Libre Franklin:bold'])
                            return fontMap['Libre Franklin:bold'];
                    ...
                    if (fontMap['Libre Franklin:normal'])
                        return fontMap['Libre Franklin:normal'];
                    return helvetica;
                }
            ...
            if (qrBox) {
                ...
                const qrPng = await qrcode_1.default.toBuffer(validationUrl, { width: qrWidth });
            ...
            if (L.nombreUsuario?.rel)
                drawTextBox(r.userName || '', L.nombreUsuario.rel, L.nombreUsuario.style || {});
            const eventTitleForThis = isPonente && r.customEventName ? r.customEventName : (r.eventName || eventName || '');
            if (L.nombreEvento?.rel)
                drawTextBox(eventTitleForThis, L.nombreEvento.rel, L.nombreEvento.style || {});
            ...
            if (L.idValidacion?.rel)
                drawTextBox(`${idValidacion}`, L.idValidacion.rel, L.idValidacion.style || {});
            if (L.ciudadFecha?.rel)
                drawTextBox(ciudadFechaText, L.ciudadFecha.rel, L.ciudadFecha.style || {});
...
exports.mergeCertificatesPdf = (0, https_1.onCall)({ cors: true, region: 'us-central1', timeoutSeconds: 540, memory: '1GiB' }, async (request) => {
    ...
    const certsSnap = await db.collection('certificados').where('nombreEvento', '==', eventName).get();
    if (certsSnap.empty)
        return { merged: false, reason: 'Sin certificados' };
    const safeEvent = eventName.replace(/[^a-zA-Z0-9_-]+/g, '_');
    ...
    if (addedPages === 0)
        return { merged: false, reason: 'Sin páginas válidas' };
...
exports.processVolunteerAttendanceCredits = (0, firestore_1.onDocumentCreated)({ document: 'volunteerAttendanceLogs/{attendanceId}', region: 'us-central1' }, async (event) => {
    ...
    if (!latest.exists || latest.get('creditsProcessed') === true)
        return;
    ...
    if (email) {
        ...
        if (!volunteerSnap.empty)
            volunteerRef = volunteerSnap.docs[0].ref;
    }
    await db.runTransaction(async (tx) => {
        const currentAttendance = await tx.get(snap.ref);
        if (!currentAttendance.exists || currentAttendance.get('creditsProcessed') === true)
            return;
        const updates = {
            creditsProcessed: true,
            creditsProcessedAt: firestore_2.FieldValue.serverTimestamp(),
        ...
        if (!validTimes) {
            ...
            tx.update(volunteerRef, {
                creditos: firestore_2.FieldValue.increment(creditsToAssign),
